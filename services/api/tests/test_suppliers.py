...
def test_post_valid_supplier(client: TestClient) -> None:
...
def test_post_missing_country(client: TestClient) -> None:
...
def test_post_usa_gbp_mismatch(client: TestClient) -> None:
...
def test_post_zero_rate(client: TestClient) -> None:
    response = client.post(
        "/api/v1/suppliers",
...
def test_post_negative_rate(client: TestClient) -> None:
    response = client.post(
        "/api/v1/suppliers",
...
def test_post_invalid_status(client: TestClient) -> None:
    response = client.post(
        "/api/v1/suppliers",
...
def test_post_empty_categories(client: TestClient) -> None:
    response = client.post(
        "/api/v1/suppliers",
...
def test_post_unknown_category(client: TestClient) -> None:
    response = client.post(
        "/api/v1/suppliers",
...
def test_post_duplicate_name(client: TestClient) -> None:
    client.post("/api/v1/suppliers", json=VALID_SUPPLIER)
    response = client.post("/api/v1/suppliers", json=VALID_SUPPLIER)
    assert response.status_code == 422
    assert "already exists" in response.json()["detail"]


def test_get_all_after_seed(client: TestClient) -> None:
    run_seed()
...
def test_get_filter_country_usa(client: TestClient) -> None:
    run_seed()
    response = client.get("/api/v1/suppliers", params={"country": "USA"})
    assert response.status_code == 200
    data = response.json()
    assert len(data) == 9
...
def test_get_filter_category_clinical_software(client: TestClient) -> None:
    run_seed()
    response = client.get("/api/v1/suppliers", params={"category": "clinical_software"})
    assert response.status_code == 200
    data = response.json()
    assert len(data) == 2
...
def test_patch_rate_zero(client: TestClient) -> None:
    created = client.post("/api/v1/suppliers", json=VALID_SUPPLIER).json()
    response = client.patch(
        f"/api/v1/suppliers/{created['id']}/rate",
        json={"monthly_rate": 0},
...
def test_patch_rate_negative(client: TestClient) -> None:
    created = client.post("/api/v1/suppliers", json=VALID_SUPPLIER).json()
    response = client.patch(
        f"/api/v1/suppliers/{created['id']}/rate",
...
def test_patch_status_suspended(client: TestClient) -> None:
    created = client.post("/api/v1/suppliers", json=VALID_SUPPLIER).json()
    response = client.patch(
...
def test_patch_status_invalid(client: TestClient) -> None:
    created = client.post("/api/v1/suppliers", json=VALID_SUPPLIER).json()
    response = client.patch(
        f"/api/v1/suppliers/{created['id']}/status",
...
def test_delete_soft_suspends(client: TestClient) -> None:
    ...
    assert response.status_code == 200
    assert response.json()["status"] == "suspended"
    still_there = client.get(f"/api/v1/suppliers/{supplier_id}")
...
def test_seeded_records_have_rate_updated_at(client: TestClient) -> None:
    run_seed()
    response = client.get("/api/v1/suppliers")
...
def test_invalid_country_query(client: TestClient) -> None:
    response = client.get("/api/v1/suppliers", params={"country": "CA"})
    assert response.status_code == 422


def test_invalid_category_query(client: TestClient) -> None:
    response = client.get("/api/v1/suppliers", params={"category": "unknown_thing"})
    assert response.status_code == 422
